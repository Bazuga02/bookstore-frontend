...
const ViewBookDetails = () => {
  ...
  useEffect(() => {
    ...
    const fetch = async () => {
      const res = await axios.get(
        `https://bookstore-q2ex.onrender.com/api/v1/get-book-by-id/${id}`
      );
  ...
  const addToFavourite = async () => {
    try {
      const response = await axios.put(
        "https://bookstore-q2ex.onrender.com/api/v1/add-to-favourite",
        {},
        { headers }
      );
      toast(
      ...
      setIsFavorite(true); // Set as favorite on successful addition
    } catch (error) {
      console.log(error);
  ...
  const addToCart = async () => {
    try {
      const response = await axios.put(
        "https://bookstore-q2ex.onrender.com/api/v1/add-to-cart",
        {},
        { headers }
      );
  ...
  const deleteBook = async () => {
    try {
      const response = await axios.delete(
        "https://bookstore-q2ex.onrender.com/api/v1/delete-book",
        { headers }
      );
      toast.success(response.data.message, {
        style: {
                    ...
                    <>
                      <button
                        className={`bg-white p-3 rounded md:rounded-full text-2xl font-semibold hover:bg-zinc-200 transition-all duration-300 flex items-center ${
                          isFavorite ? "text-pink-400" : "text-black"
                        }`}
